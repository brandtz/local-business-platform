...
function buildProgressBarStep(step: OrderTrackingStepInfo, now?: number): OrderProgressBarStep {
  return {
    label: step.label,
    state: step.state,
...
function buildCustomerInfoCard(data: OrderTrackingData): CustomerInfoCardViewModel {
  ...
  return {
    ...
    deliveryAddress: card.deliveryAddress ? formatDeliveryAddress(card.deliveryAddress) : null,
  };
}
...
function buildReceipt(data: OrderTrackingData): OrderReceiptViewModel {
  return {
    ...
    deliveryFeeFormatted: formatReceiptCents(data.receipt.deliveryFeeCents),
