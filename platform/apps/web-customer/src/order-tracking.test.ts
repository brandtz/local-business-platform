...
import { buildOrderTrackingViewModel, formatReceiptCents } from "./order-tracking";

function sampleTrackingData(overrides?: Partial<OrderTrackingData>): OrderTrackingData {
  return {
    orderId: "order-1",
    status: "placed",
    isCancelled: false,
...
describe("Order Tracking", () => {
  ...
  describe("buildOrderTrackingViewModel", () => {
    it("builds complete view model for placed order", () => {
      const vm = buildOrderTrackingViewModel(sampleTrackingData(), baseTime);
      ...
      expect(vm.statusMessage).toBe("Your order has been placed and is waiting to be confirmed.");
    });
    ...
    it("builds customer info card for delivery", () => {
      ...
      expect(vm.customerInfo.deliveryAddress).toBe("123 Main St, Apt 4, Springfield, IL 62701");
    });
