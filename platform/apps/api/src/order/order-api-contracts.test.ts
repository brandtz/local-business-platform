...
describe("Order API Contracts", () => {
	...
	describe("assertValidPlaceOrderRequest", () => {
		it("accepts valid request with cartSessionId", () => {
			expect(() =>
				assertValidPlaceOrderRequest({ cartSessionId: "cart-1" })
			).not.toThrow();
		});
		...
		it("rejects non-object", () => {
			expect(() =>
				assertValidPlaceOrderRequest("not-object")
			).toThrow(OrderApiContractError);
		});

		it("rejects null", () => {
			expect(() =>
				assertValidPlaceOrderRequest(null)
			).toThrow(OrderApiContractError);
		});
		...
		it("rejects empty cartSessionId", () => {
			expect(() =>
				assertValidPlaceOrderRequest({ cartSessionId: "" })
			).toThrow(OrderApiContractError);
		});
	...
	describe("assertValidTransitionOrderStatusRequest", () => {
		...
		it("rejects non-object", () => {
			expect(() =>
				assertValidTransitionOrderStatusRequest("not-object")
			).toThrow(OrderApiContractError);
		});
	...
	describe("assertValidCancelOrderRequest", () => {
		...
		it("accepts with reason", () => {
			expect(() =>
				assertValidCancelOrderRequest({ reason: "Changed my mind" })
			).not.toThrow();
		});

		it("rejects non-object", () => {
			expect(() =>
				assertValidCancelOrderRequest("not-object")
			).toThrow(OrderApiContractError);
		});

		it("rejects non-string reason", () => {
			expect(() =>
				assertValidCancelOrderRequest({ reason: 123 })
			).toThrow(OrderApiContractError);
		});
	});
	...
	describe("validateAdminOrderListQuery", () => {
		...
		it("rejects invalid status filter", () => {
			expect(() =>
				validateAdminOrderListQuery({ status: "fake" })
			).toThrow(OrderApiContractError);
		});
		...
		it("rejects invalid fulfillmentMode filter", () => {
			expect(() =>
				validateAdminOrderListQuery({ fulfillmentMode: "fake" })
			).toThrow(OrderApiContractError);
		});
		...
		it("rejects invalid page", () => {
			expect(() =>
				validateAdminOrderListQuery({ page: "0" })
			).toThrow(OrderApiContractError);

			expect(() =>
				validateAdminOrderListQuery({ page: "abc" })
			).toThrow(OrderApiContractError);
		});
		...
		it("rejects invalid pageSize", () => {
			expect(() =>
				validateAdminOrderListQuery({ pageSize: "0" })
			).toThrow(OrderApiContractError);

			expect(() =>
				validateAdminOrderListQuery({ pageSize: "101" })
			).toThrow(OrderApiContractError);
		});
