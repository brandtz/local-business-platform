...
function sampleOrderInput(overrides?: Partial<CreateOrderFromCartInput>): CreateOrderFromCartInput {
  return {
    tenantId: TENANT_A,
    customerId: "cust-1",
...
describe("OrderService", () => {
  ...
  describe("createOrderFromCart", () => {
    ...
    it("rejects order with no items", () => {
      expect(() => service.createOrderFromCart(sampleOrderInput({ items: [] }))).toThrow(
        OrderValidationError
      );
    });

    it("rejects order with negative total", () => {
      expect(() => service.createOrderFromCart(sampleOrderInput({ totalCents: -1 }))).toThrow(
        OrderValidationError
      );
    });
    ...
    it("captures promo code", () => {
      const order = service.createOrderFromCart(sampleOrderInput({ promoCode: "SAVE20" }));
  ...
  describe("transitionOrderStatus", () => {
    it("transitions placed → confirmed", () => {
      const order = service.createOrderFromCart(sampleOrderInput());
      const updated = service.transitionOrderStatus(TENANT_A, order.id, "confirmed");
    ...
    it("transitions confirmed → preparing", () => {
      ...
      const updated = service.transitionOrderStatus(TENANT_A, order.id, "preparing");
    ...
    it("transitions preparing → ready", () => {
      ...
      const updated = service.transitionOrderStatus(TENANT_A, order.id, "ready");
    ...
    it("transitions ready → completed", () => {
      ...
      const updated = service.transitionOrderStatus(TENANT_A, order.id, "completed");
    ...
    it("rejects invalid transition placed → ready", () => {
      const order = service.createOrderFromCart(sampleOrderInput());
      expect(() => service.transitionOrderStatus(TENANT_A, order.id, "ready")).toThrow(
        OrderTransitionError
      );
    });
    ...
    it("rejects backward transition confirmed → placed", () => {
      const order = service.createOrderFromCart(sampleOrderInput());
      service.transitionOrderStatus(TENANT_A, order.id, "confirmed");
      expect(() => service.transitionOrderStatus(TENANT_A, order.id, "placed")).toThrow(
        OrderTransitionError
      );
    });
    ...
    it("rejects transition from cancelled", () => {
      ...
      expect(() => service.transitionOrderStatus(TENANT_A, order.id, "confirmed")).toThrow(
        OrderTransitionError
      );
    });

    it("throws for non-existent order", () => {
      expect(() => service.transitionOrderStatus(TENANT_A, "fake-id", "confirmed")).toThrow(
        OrderNotFoundError
      );
    });
    ...
    it("enforces tenant isolation on transitions", () => {
      const order = service.createOrderFromCart(sampleOrderInput());
      expect(() => service.transitionOrderStatus(TENANT_B, order.id, "confirmed")).toThrow(
        OrderNotFoundError
      );
    });
  });
  ...
  describe("cancelOrder", () => {
    it("cancels a placed order", () => {
      const order = service.createOrderFromCart(sampleOrderInput());
      const cancelled = service.cancelOrder(TENANT_A, order.id, "Customer request");
    ...
    it("rejects cancellation of ready order", () => {
      ...
      service.transitionOrderStatus(TENANT_A, order.id, "preparing");
      service.transitionOrderStatus(TENANT_A, order.id, "ready");
      expect(() => service.cancelOrder(TENANT_A, order.id)).toThrow(OrderTransitionError);
    });
  ...
  describe("admin queries", () => {
    it("lists all orders for a tenant", () => {
      ...
      service.createOrderFromCart(sampleOrderInput({ customerId: "cust-2", customerName: "Jane" }));
    ...
    it("filters orders by status", () => {
      ...
      service.createOrderFromCart(sampleOrderInput({ customerId: "cust-2" }));
    ...
    it("paginates results", () => {
      for (let i = 0; i < 5; i++) {
        service.createOrderFromCart(sampleOrderInput({ customerId: `cust-${i}` }));
      }
    ...
    it("throws for admin detail of non-existent order", () => {
      expect(() => service.getAdminOrderDetail(TENANT_A, "fake-id")).toThrow(OrderNotFoundError);
    });

    it("enforces tenant isolation on admin detail", () => {
      const order = service.createOrderFromCart(sampleOrderInput());
      expect(() => service.getAdminOrderDetail(TENANT_B, order.id)).toThrow(OrderNotFoundError);
    });
  });
  ...
  describe("pipeline counts", () => {
    it("returns counts for all statuses", () => {
      service.createOrderFromCart(sampleOrderInput());
      const order2 = service.createOrderFromCart(sampleOrderInput({ customerId: "cust-2" }));
      ...
      const placedCount = pipeline.counts.find((c) => c.status === "placed");
  ...
  describe("customer queries", () => {
    ...
    it("returns customer order detail without admin fields", () => {
      const order = service.createOrderFromCart(sampleOrderInput());
      const detail = service.getCustomerOrderDetail(TENANT_A, order.id, "cust-1");
      expect(detail.status).toBe("placed");
    ...
    it("throws for customer accessing another customer's order", () => {
      const order = service.createOrderFromCart(sampleOrderInput());
      expect(() => service.getCustomerOrderDetail(TENANT_A, order.id, "cust-999")).toThrow(
        OrderNotFoundError
      );
    });
    ...
    it("enforces tenant isolation on customer queries", () => {
      const order = service.createOrderFromCart(sampleOrderInput());
      expect(() => service.getCustomerOrderDetail(TENANT_B, order.id, "cust-1")).toThrow(
        OrderNotFoundError
      );
    });
  });
  ...
  describe("order tracking", () => {
    it("returns tracking data for a placed order", () => {
      const order = service.createOrderFromCart(sampleOrderInput());
      const tracking = service.getOrderTrackingData(TENANT_A, order.id, "cust-1");
    ...
    it("includes delivery address in customer info card", () => {
      ...
      expect(tracking.customerInfoCard.deliveryAddress?.line1).toBe("123 Main St");
    });

    it("throws for customer accessing another customer's tracking", () => {
      const order = service.createOrderFromCart(sampleOrderInput());
      expect(() => service.getOrderTrackingData(TENANT_A, order.id, "cust-999")).toThrow(
        OrderNotFoundError
      );
    });
  });
