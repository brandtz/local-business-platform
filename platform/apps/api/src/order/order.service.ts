...
export class OrderService {
  ...
  transitionOrderStatus(
    ...
    if (targetStatus === "cancelled" && !isOrderCancellable(order.status)) {
      throw new OrderTransitionError(`Order in status '${order.status}' cannot be cancelled.`);
    }
  ...
  cancelOrder(tenantId: string, orderId: string, reason?: string): OrderRecord {
    return this.transitionOrderStatus(tenantId, orderId, "cancelled", reason);
  }
  ...
  getAdminOrderDetail(tenantId: string, orderId: string): AdminOrderDetail {
    const order = this.repository.getOrderById(tenantId, orderId);
    if (!order) {
      throw new OrderNotFoundError(`Order ${orderId} not found.`);
    }
  ...
  listAdminOrders(query: AdminOrderListQuery): AdminOrderListResponse {
    const allOrders = this.repository.listOrdersForTenant(query.tenantId, {
      status: query.status,
  ...
  getCustomerOrderDetail(
    ...
    const order = this.repository.getOrderByIdForCustomer(tenantId, orderId, customerId);
    if (!order) {
      throw new OrderNotFoundError(`Order ${orderId} not found.`);
    }
  ...
  listCustomerOrders(tenantId: string, customerId: string): CustomerOrderSummary[] {
    const orders = this.repository.listOrdersForCustomer(tenantId, customerId);
  ...
  getOrderTrackingData(tenantId: string, orderId: string, customerId: string): OrderTrackingData {
    const order = this.repository.getOrderByIdForCustomer(tenantId, orderId, customerId);
    if (!order) {
      throw new OrderNotFoundError(`Order ${orderId} not found.`);
    }
  ...
  private buildCustomerOrderSummary(order: OrderRecord): CustomerOrderSummary {
    const items = this.repository.listOrderItems(order.id);
    return {
      id: order.id,
      createdAt: order.createdAt,
  ...
  private buildCustomerOrderDetail(order: OrderRecord): CustomerOrderDetail {
    const items = this.repository.listOrderItems(order.id);
    const itemDetails = items.map((item) => this.buildOrderItemDetail(item));
  ...
  private buildOrderItemModifierDetail(modifier: OrderItemModifierRecord): OrderItemModifierDetail {
    return {
      id: modifier.id,
